package com.dreamteam.httprequest.Data;

public class ConstantConfig {
    public final String IMAGE_TYPE =                        "image";
    public final String ADMIN =                             "Admin";
    public final String USER_TYPE =                         "User";
    public final String VOITED =                            "voited";
    public final String GROUP_TYPE =                        "Group";
    public final String GET_GROUPS_TYPE =                    "getGroups";
    public final String GET_GROUP_TYPE =                    "getGroup";

    public final String DELETE_GROUP =                      "delete";
    public final String ADD =                               "Add";
    public final String DELETE =                            "Delete";
    public final String MEMBERS_TYPE =                      "members";

    public final String GET_USERS_FOR_SELECT_DELETE_TYPE =  "delete users";
    public final String SET_USER_IN_GROUP_TYPE =            "get user in group";
    public final String SET_DELETE_USER_IN_GROUP_TYPE =     "delete user in group";
    public final String POST_GROUP =                        "postGroup";
    public final String PREFIX =                            "data:image/jpeg;base64,";
    public final String MEMBERS_FOR_LIST_TYPE =             "members for list";
    public final String POST_USER =                         "postUser";
    public final String PUT_USER =                          "putUser";
    public final String GET_GROUP_FOR_LIST_TYPE =           "getGroup for list";
    public final String EXIT_USER_IN_GROUP_TYPE =           "exit group";
    public final String GET_EVENT_TYPE =                    "getEvent";
    public final String GET_INVITATION_TYPE =               "getInvitation";
    public final String GET_INVITATIONS_TYPE =              "getInvitations";
    public final String GET_GROUP_AFTER_EDIT_TYPE =         "public after edit";
    public final String ADD_ADMIN =                         "Add admin";
    public final String ADD_GROUP_TYPE =                    "Add group";
    public final String EDIT_GROUP_TYPE =                   "Edit group";
    public final String ANSWER_FOR_EVENT_TYPE =             "Answer for event";
    public final String RESULT_TO_QUESTION_TYPE =           "Result To Question";
    public final String GET_SUB_GROUP_TYPE =                "Get subgroup";
    public final String ACTIVATED_GROUP_TYPE =              "Activated group";
    public final String EVENT_TYPE_RATING =                 "rating";
    public final String EVENT_TYPE_DISCUSSION =             "discussion";
    public final String EVENT_TYPE_POLL =                   "poll";
    public final String EVENTS_TYPE =                       "get events";
    public final String USERS_FOR_VOTING =                  "Users for voting";

    public final String GET_USERS_FOR_ADD_STEP_1_TYPE =     "add users";
    public final String GET_USERS_FOR_ADD_STEP_2_TYPE =     "Get user";

    public final String GET_RATING_TYPE =                   "Get rating";
    public final String GET_GROUP_FOR_INVITATION_TYPE =     "Get group for invitation";
    public final String GET_USER_FOR_INVITATION_TYPE =      "Get user for invitation";

    public final String CREATE_LOGIN =                     "create login";
    public final String ENABLE_USER_AUTH =                 "enable user auth";
    public final String GET_USER_TOKEN =                   "get user token";
    public final String ADD_USER_AUTH =                    "add user auth";

    public final String SUBSCRIPTION =                      "subscription";
    public final String ACTIVATION_SUB_GROUP =              "activation subgroup";

    public final String CONFIRMED =                         "confirmed";
    public final String DENIED =                            "denied";
}

